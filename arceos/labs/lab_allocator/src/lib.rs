...
impl LabByteAllocator {
    ...
    fn is_bump_candidate(&self, layout: Layout) -> bool {
        layout.size() >= 2048 || layout.align() > 64
    }
}
...
impl BaseAllocator for LabByteAllocator {
    fn init(&mut self, start: usize, size: usize) {
        ...
        let buddy_size = size / 2;  // 分配一半给buddy
    ...
    fn add_memory(&mut self, start: usize, size: usize) -> AllocResult {
        if DEBUG {
            axlog::ax_println!("{}add_memory bump_end:{} start:{} size:{}{}, whoneed:{}", BLUE, self.bump_end, start, size, RESET, self.who_need_memory);
        }
...
impl ByteAllocator for LabByteAllocator {
    fn alloc(&mut self, layout: Layout) -> AllocResult<NonNull<u8>> {
        ...
        if DEBUG {
            axlog::ax_println!("pos: {:?}, alloc: layout:{:?}, align:{:?}", pos, layout, align );
        }
        // axlog::ax_println!("{}counter:{}{}-{}size:{}{}", RED, self.counter, RESET, GREEN, size, RESET);
        pos
    }
    ...
    fn dealloc(&mut self, pos: NonNull<u8>, layout: Layout) {
        ...
        if DEBUG {
            axlog::ax_println!("dealloc: {}layout:{:?}{}, pos:{:?}", RED,layout, RESET ,pos);
        }
